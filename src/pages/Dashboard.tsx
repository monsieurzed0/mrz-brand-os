...
export default function Dashboard() {
  ...
  if (loading) {
    return <div className="p-6 text-sm text-[#A1A1AA]">Chargement du dashboard...</div>;
  }

  if (error) {
    return <div className="p-6 text-sm text-red-400">Erreur dashboard : {error}</div>;
  }

  if (!data) {
    return <div className="p-6 text-sm text-[#71717A]">Aucune donnée dashboard</div>;
  }
    ...
    <div className="space-y-8">
      <section className="rounded-3xl border border-white/10 bg-[#141416] p-6">
        ...
        <div className="mt-4 text-xs text-[#71717A]">
          {weekly?.week_label || "Semaine non définie"}
        </div>
      </section>
