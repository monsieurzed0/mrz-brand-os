...
export default function VisualLab() {
  ...
  const {
    data: visualPromptsData,
    setData: setVisualPromptsData,
  } = useApiQuery(api.getVisualPrompts, []);
  ...
  const buildVisualResult = (): VisualResult => {
    return {
      hookVisual: `Bold typographic composition: "${form.subject
        .split(' ')
        .slice(0, 6)
        .join(' ')}" — Raleway ExtraBold, #F0EDE8 on #0D0D10, copper accent line #D67A2C, minimal asymmetric layout, ${form.format}`,
      ...
      negativePrompt:
        'text, watermark, logo, bright colors, blue tones, purple, neon, green glow, cartoon, illustration, anime, low quality, blurry, generic stock photo, smiling business people, laptops, white background, startup cliché, crypto aesthetic',
    ...
    <div>
      ...
      <div className="p-6 space-y-5 animate-fade-in">
        ...
        <SectionCard title="Brief visuel">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <div className="lg:col-span-2">
              ...
              <input
                ...
                placeholder="Ex: L'excellence du branding africain"
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Éditorial, cinématique"
                className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30"
              />
            </div>
            ...
            <div className="lg:col-span-3">
              ...
              <input
                ...
                placeholder="Ex: Post Instagram premium pour acquisition leads"
                className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30"
              />
            </div>
          </div>
          ...
          <button
            onClick={generate}
            className="mt-4 flex items-center gap-2 px-5 py-2.5 rounded-lg bg-copper text-dark text-sm font-bold hover:bg-copper-light transition"
          >
            ...
            <div className="flex gap-2">
              {moodColors.map((c) => (
                <div key={c} className="flex-1">
                  <div
                    className="h-12 rounded-lg border border-exec/10"
          ...
          <SectionCard title="Formats recommandés">
            <div className="grid grid-cols-2 gap-2">
              {formats.map((f) => (
                <div
                  key={f}
                  className="p-2 rounded bg-deep border border-exec/5 text-xs text-muted text-center"
                >
                  {f}
                </div>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-copper/20 bg-copper/5 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copy(displayResult.hookVisual)}
                  className="text-xs text-muted hover:text-copper flex items-center gap-1"
                >
                  <Copy size={11} /> Copier
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {[
                ...
                <div
                  key={p.label}
                  className="rounded-xl border border-exec/10 bg-carbon p-4 group"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-ivory">{p.label}</span>
                    <button
                      onClick={() => copy(p.content)}
                      className="opacity-0 group-hover:opacity-100 transition text-xs text-muted hover:text-copper flex items-center gap-1"
                    >
                      <Copy size={11} /> Copier
                    </button>
                  </div>
            ...
            <div className="rounded-xl border border-red-900/20 bg-red-950/10 p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copy(displayResult.negativePrompt)}
                  className="text-xs text-muted hover:text-copper flex items-center gap-1"
                >
                  <Copy size={11} /> Copier
                </button>
            ...
            <SectionCard title="Note Photoshop">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  onClick={() => copy(displayResult.notePS)}
                  className="text-xs text-muted hover:text-copper flex items-center gap-1"
                >
                  <Copy size={11} /> Copier
                </button>
