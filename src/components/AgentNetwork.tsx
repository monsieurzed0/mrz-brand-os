...
/* ------------------------------------------------------------------ */
/*  Sub-component : travelling packet                                */
/* ------------------------------------------------------------------ */

function TravellingPacket({
  startX, startY, endX, endY, duration, label, status,
}: Packet) {
...
export default function AgentNetwork({ events, activeAgentIds }: AgentNetworkProps) {
  ...
  useEffect(() => {
    ...
    /* Nettoyer les IDs obsolètes pour éviter la fuite mémoire */
    ...
    events.forEach(event => {
      ...
      if (event.type === 'packet' && event.from && event.to) {
  ...
  if (activeAgentIds.length > 0) activeNodes.add('backend');
