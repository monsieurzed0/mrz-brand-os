...
import type { ContentIdea, Product, Platform, Duration, ContentIdeaStatus } from '@/types';

const IDEA_STATUSES: ContentIdeaStatus[] = ['idea_pending', 'idea_ready', 'script_pending', 'archived'];

export default function ContentLab() {
  const { state, addContentIdea, updateContentIdea, deleteContentIdea, showToast } = useStore();
  ...
  const [form, setForm] = useState<Partial<ContentIdea>>({
    subject: '', angle: '', target: '', product: 'Mr Z Brand', platform: 'TikTok', duration: 60, cta: '', source: '', status: 'idea_pending',
  });

  const ideas = state.contentIdeas.filter(i => {
  ...
  const handleSave = () => {
    ...
    if (editId) {
      ...
      addContentIdea(form as Omit<ContentIdea, 'id' | 'createdAt'>);
    }
    setForm({ subject: '', angle: '', target: '', product: 'Mr Z Brand', platform: 'TikTok', duration: 60, cta: '', source: '', status: 'idea_pending' });
    setShowForm(false);
  };
  ...
  const generateIdeas = () => {
    ...
    subjects.forEach((s, i) => {
      addContentIdea({
        ...
        angle: angles[i],
        target: targets[i],
        product: PRODUCTS[i % 3] as Product,
        platform: PLATFORMS[i % 5] as Platform,
  ...
  const editIdea = (idea: ContentIdea) => {
    setForm(idea);
    setEditId(idea.id);
    setShowForm(true);
  };
      ...
      <div className="p-6 space-y-5 animate-fade-in">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-subtle bg-deep px-2 py-0.5 rounded-full">{state.contentIdeas.length} idées</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={generateIdeas} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-copper/15 border border-copper/30 text-copper-light text-sm font-semibold hover:bg-copper/25 transition">
              <Sparkles size={14} /> Générer 5 idées IA
            </button>
            <button onClick={() => { setEditId(null); setForm({ subject: '', angle: '', target: '', product: 'Mr Z Brand', platform: 'TikTok', duration: 60, cta: '', source: '', status: 'idea_pending' }); setShowForm(true); }} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-carbon border border-exec/15 text-muted text-sm font-medium hover:border-copper/30 transition">
              <Plus size={14} /> Nouvelle idée
            </button>
            <div className="flex border border-exec/15 rounded-lg overflow-hidden">
              <button onClick={() => setView('table')} className={`p-2 ${view === 'table' ? 'bg-copper/15 text-copper' : 'text-subtle hover:text-muted'}`}>
                <List size={14} />
              </button>
              <button onClick={() => setView('kanban')} className={`p-2 ${view === 'kanban' ? 'bg-copper/15 text-copper' : 'text-subtle hover:text-muted'}`}>
                <Grid3X3 size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <select value={filterProduct} onChange={e => setFilterProduct(e.target.value)} className="bg-deep border border-exec/15 rounded-lg px-3 py-1.5 text-sm text-ivory focus:outline-none focus:border-copper/30">
            <option value="">Tous les produits</option>
            {PRODUCTS.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
          <select value={filterPlatform} onChange={e => setFilterPlatform(e.target.value)} className="bg-deep border border-exec/15 rounded-lg px-3 py-1.5 text-sm text-ivory focus:outline-none focus:border-copper/30">
            <option value="">Toutes les plateformes</option>
            {PLATFORMS.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="bg-deep border border-exec/15 rounded-lg px-3 py-1.5 text-sm text-ivory focus:outline-none focus:border-copper/30">
        ...
        {showForm && (
          <SectionCard title={editId ? 'Modifier l\'idée' : 'Nouvelle idée'}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <div><label className="text-xs text-subtle font-semibold">Angle</label><input value={form.angle || ''} onChange={e => setForm({...form, angle: e.target.value})} className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30" /></div>
              <div><label className="text-xs text-subtle font-semibold">Cible</label><input value={form.target || ''} onChange={e => setForm({...form, target: e.target.value})} className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30" /></div>
              ...
              <div><label className="text-xs text-subtle font-semibold">Durée (s)</label><select value={form.duration} onChange={e => setForm({...form, duration: Number(e.target.value) as Duration})} className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30">{DURATIONS.map(d => <option key={d} value={d}>{d}s</option>)}</select></div>
              <div><label className="text-xs text-subtle font-semibold">CTA</label><input value={form.cta || ''} onChange={e => setForm({...form, cta: e.target.value})} className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30" /></div>
              ...
              <div><label className="text-xs text-subtle font-semibold">Statut</label><select value={form.status} onChange={e => setForm({...form, status: e.target.value as ContentIdeaStatus})} className="w-full mt-1 bg-deep border border-exec/15 rounded-lg px-3 py-2 text-sm text-ivory focus:outline-none focus:border-copper/30">{IDEA_STATUSES.map(s => <option key={s} value={s}>{STATUS_MAP[s]}</option>)}</select></div>
            </div>
            <div className="flex gap-2 mt-4">
              <button onClick={handleSave} className="px-4 py-2 rounded-lg bg-copper text-dark text-sm font-bold hover:bg-copper-light transition">{editId ? 'Mettre à jour' : 'Ajouter'}</button>
                    ...
                    <tr key={idea.id} className="hover:bg-carbon/40 transition">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          <button onClick={() => editIdea(idea)} className="text-xs text-muted hover:text-copper transition px-2 py-1 rounded hover:bg-copper/10">Éditer</button>
                          <button onClick={() => deleteContentIdea(idea.id)} className="text-xs text-subtle hover:text-red-400 transition px-2 py-1 rounded hover:bg-red-900/10">Suppr.</button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {IDEA_STATUSES.map(status => (
              <div key={status} className="rounded-xl border border-exec/10 bg-carbon">
                <div className="px-4 py-3 border-b border-exec/10 flex items-center justify-between">
                  <span className="text-xs font-bold text-ivory uppercase tracking-wider">{STATUS_MAP[status]}</span>
                ...
                <div className="p-3 space-y-2 min-h-[100px]">
                  {ideas.filter(i => i.status === status).map(idea => (
                    <div key={idea.id} onClick={() => editIdea(idea)} className="p-3 rounded-lg bg-deep border border-exec/5 hover:border-copper/20 cursor-pointer transition">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <SectionCard title="Distribution par produit">
            <div className="space-y-2">
              {PRODUCTS.map(p => {
                const count = state.contentIdeas.filter(i => i.product === p).length;
                const pct = state.contentIdeas.length > 0 ? (count / state.contentIdeas.length) * 100 : 0;
                return (
          ...
          <SectionCard title="Distribution par plateforme">
            <div className="space-y-2">
              {PLATFORMS.map(p => {
                const count = state.contentIdeas.filter(i => i.platform === p).length;
                const pct = state.contentIdeas.length > 0 ? (count / state.contentIdeas.length) * 100 : 0;
                return (
                  <div key={p} className="flex items-center gap-3">
                    <span className="text-xs text-muted w-32 truncate">{p}</span>
